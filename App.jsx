...
const SectionHead = ({children,action}) => (<div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:16}}><h3 style={{margin:0,fontSize:17,fontWeight:700,color:"#1a3d2b"}}>{children}</h3>{action}</div>);
    ...
    <div style={{borderTop:"2px solid #e8d5f0",paddingTop:20,marginTop:8}}>
      ...
      {showFertil&&<><div style={{display:"flex",gap:12,flexWrap:"wrap",marginBottom:20}}>{statCard("💜","Pacientes activas",fertilActivas,"en programa","#7b2d8b")}{statCard("✅","Finalizadas",fertilFinalizadas,"completaron")}{statCard("💰","Ingresos Fértil",fmtMoney(fertilTotalIngresos),"total acumulado","#7b2d8b")}{statCard("🎫","Ticket promedio",fmtMoney(fertilTicketPromedio),"por paciente")}</div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16,marginBottom:16}}>
          <div style={S.card}><h4 style={{margin:"0 0 12px",fontSize:14,fontWeight:700,color:C.fertil}}>Ingresos Fértil por mes</h4>{last6Fertil.some(d=>d.value>0)?<BarChart data={last6Fertil} color="#7b2d8b" formatValue={v=>`$${Math.round(v/1000)}k`}/>:<p style={{fontSize:13,color:"#aaa",textAlign:"center",padding:"30px 0"}}>Sin datos aún</p>}</div>
...
function FertilDashboard({fertilCases,patients,appointments,onSelectCase,onNewCase}){
  ...
  return(<div>
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16,marginBottom:20}}>
      <div style={S.card}><h4 style={{margin:"0 0 12px",fontSize:14,fontWeight:700,color:C.fertil}}>{"📅 Próximos turnos Fértil"}</h4>{proximosTurnos.length===0?<p style={{fontSize:12,color:"#aaa",textAlign:"center",padding:"20px 0"}}>Sin turnos programados</p>:proximosTurnos.map(renderTurno)}</div>
      <div style={S.card}><h4 style={{margin:"0 0 12px",fontSize:14,fontWeight:700,color:C.danger}}>⚠️ Alertas</h4>{alertas.length===0?<p style={{fontSize:12,color:"#aaa",textAlign:"center",padding:"20px 0"}}>Sin alertas</p>:alertas.map((a,i)=>(<div key={i} onClick={()=>onSelectCase(a.caseId)} style={{padding:"8px 10px",background:"#fff5f0",borderRadius:8,marginBottom:6,fontSize:12,color:"#c0392b",cursor:"pointer",fontWeight:600}}>{a.msg}</div>))}</div>
    </div>
...
function PatientDetail({patient,dispatch,consultas,eventos,appointments,fertilCases,onAddConsulta,onDeleteConsulta,onAddEvento,onUpdateEvento,onDeleteEvento,onGeneratePlan,onBack,onDelete,onGoToFertil}) {
  ...
  {tab==="resumen"&&<div>
    ...
    <div style={{...S.card}}>
      ...
      </div>:<p style={{color:"#aaa",fontSize:13,textAlign:"center",padding:"20px 0"}}>Sin mediciones aún</p>}
    </div>
  </div>}
