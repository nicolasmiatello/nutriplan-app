...
async function sbLoadConsultas() {
  const r = await fetch(`${SUPABASE_URL}/rest/v1/consultas?select=*&order=fecha.desc`, {
    headers: sbHeaders
  });
  if (!r.ok) return [];
  const rows = await r.json();
 ...
 useEffect(() => {
  ...
  const t = setTimeout(async () => {
    try {
      ...
      // 👇 NUEVO: guardar consultas
      if (state.consultas && state.consultas.length > 0) {
        await Promise.all(state.consultas.map(c => sbInsertConsulta(c)));
      }
...
}, [state.patients, state.consultas, loaded]);
