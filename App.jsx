    ...
    <div style={{borderTop:"2px solid #e8d5f0",paddingTop:20,marginTop:8}}>
      ...
      {showFertil&&<><div style={{display:"flex",gap:12,flexWrap:"wrap",marginBottom:20}}>{statCard("💜","Pacientes activas",fertilActivas,"en programa","#7b2d8b")}{statCard("✅","Finalizadas",fertilFinalizadas,"completaron")}{statCard("💰","Ingresos Fértil",fmtMoney(fertilTotalIngresos),"total acumulado","#7b2d8b")}{statCard("🎫","Ticket promedio",fmtMoney(fertilTicketPromedio),"por paciente")}</div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:18,marginBottom:16}}>
          <div style={S.card}><h4 style={{margin:"0 0 12px",fontSize:14,fontWeight:700,color:C.fertil}}>Ingresos Fértil por mes</h4>{last6Fertil.some(d=>d.value>0)?<BarChart data={last6Fertil} color="#7b2d8b" formatValue={v=>`$${Math.round(v/1000)}k`}/>:<EmptyState icon="📊" title="Sin datos aún" compact/>}</div>
...
function FertilDashboard({fertilCases,patients,appointments,onSelectCase,onNewCase}){
  ...
  return(<div>
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:18,marginBottom:20}}>
      <div style={S.card}><h4 style={{margin:"0 0 12px",fontSize:14,fontWeight:700,color:C.fertil}}>{"📅 Próximos turnos Fértil"}</h4>{proximosTurnos.length===0?<EmptyState icon="📅" title="Sin turnos programados" compact/>:proximosTurnos.map(renderTurno)}</div>
      <div style={S.card}><h4 style={{margin:"0 0 12px",fontSize:14,fontWeight:700,color:C.danger}}>⚠️ Alertas</h4>{alertas.length===0?<EmptyState icon="✨" title="Sin alertas" sub="Todo en orden" compact/>:alertas.map((a,i)=>(<div key={i} onClick={()=>onSelectCase(a.caseId)} style={{padding:"8px 10px",background:"#fff5f0",borderRadius:8,marginBottom:6,fontSize:12,color:"#c0392b",cursor:"pointer",fontWeight:600}}>{a.msg}</div>))}</div>
    </div>
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:18,marginBottom:20}}>
      ...
      <div>
        <div style={{...S.card,marginBottom:18}}>
          ...
          {totalHoy===0?<EmptyState icon="☀️" title="Día libre" sub="No hay turnos para hoy" compact/>:<div>
...
function PatientDetail({patient,dispatch,consultas,eventos,appointments,fertilCases,onAddConsulta,onDeleteConsulta,onAddEvento,onUpdateEvento,onDeleteEvento,onGeneratePlan,onBack,onDelete,onGoToFertil}) {
  ...
  {tab==="resumen"&&<div>
    ...
    <div style={{...S.card}}>
      ...
      </div>:<EmptyState icon="📏" title="Sin mediciones aún" sub="Registrá peso, grasa y masa muscular" compact/>}
    </div>
  </div>}

  {tab==="evol"&&<div><div style={{...S.card,marginBottom:18}}><SectionHead>Agregar nota de evolución</SectionHead><textarea value={newNota} onChange={e=>setNewNota(e.target.value)} rows={3} placeholder="Notas de la consulta de hoy..." style={{...S.input,resize:"vertical",marginBottom:10}}/><button onClick={addNota} disabled={!newNota.trim()} style={{...S.btnPrimary,width:"100%",opacity:newNota.trim()?1:.5}}>{"💾 Guardar nota"}</button></div>{patient.notas?.length===0?<EmptyState icon="📝" title="Sin notas aún" sub="Las notas de evolución aparecerán acá" compact/>:patient.notas?.map(n=>(<div key={n.id} style={{...S.card,marginBottom:10,borderLeft:"3px solid #52b788"}}><div style={{fontSize:11,color:"#7a9a8a",marginBottom:6,fontWeight:600}}>{n.fecha}</div><p style={{margin:0,fontSize:14,color:"#2a2a2a",lineHeight:1.6}}>{n.texto}</p></div>))}</div>}
