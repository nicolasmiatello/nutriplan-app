...
function reducer(state,action) {
  switch(action.type) {
    ...
    case "UPDATE_PATIENT_DATA": return {...state,patients:state.patients.map(p=>p.id===action.pid?{...p,...action.data}:p)};
...
function CalendarView({eventos,patients,appointments,onAddEvento,onUpdateEvento,onDeleteEvento,onSelectPatient,filterPatientId}) {
  ...
  return (<div>
    ...
    {viewMode==="month"&&<div>
      ...
      <div style={{background:"#fff",borderRadius:14,border:"1px solid #E5E7EB",boxShadow:"0 1px 3px rgba(0,0,0,0.04)",padding:12,width:"100%"}}>
        <div style={{display:"grid",gridTemplateColumns:"5fr 5fr 5fr 5fr 5fr 1.2fr 1.2fr",gap:2}}>
